...
selected_scc <- filter(SCC, grepl("coal",Short.Name, ignore.case = TRUE)  )

# Now select all the rows from NEI that matches with SCC code in SCC dataset
filtered_NEI <- filter(NEI, NEI$SCC %in% selected_scc$SCC)
...
title(main = "Total PM 2.5 Emissions per Year for Coal related source",col.main="blue")
dev.off()
